...
define('Flux', [], function(require, exports, module) {
  (function ($) {
    flux.slider = function (elem, opts) {
      ...
      this.image1 = $('<div class="image1"></div>').appendTo(this.imageContainer);
      this.image2 = $('<div class="image2"></div>').appendTo(this.imageContainer);
      ...
      this.element.bind("mousedown touchstart", function (event) {
        "touchstart" == event.type ? i.left = event.targetTouches[0].pageX : "mousedown" == event.type && (i.left = event.pageX)
      }).bind("mouseup touchend", function (event) {
        "touchend" == event.type ? j.left = event.changedTouches[0].pageX : "mouseup" == event.type && (j.left = event.pageX), j.left - i.left > k ? _this.prev(null, {direction: "right"}) : i.left - j.left > k && _this.next(null, {direction: "left"}), _this.options.autoplay && (_this.stop(), _this.start())
      });
    ...
    flux.slider.prototype = {
      ...
      setupImages: function () {
        var img1 = this.getImage(this.currentImageIndex),
          css1 = {
            'background': 'url("' + img1.src + '") 50% 50% / contain no-repeat rgba(255, 255, 255, 0)',
            'z-index': 101,
            'cursor': 'auto'
          };
        ...
        this.image2.css({
          'background': 'url("' + this.getImage(this.nextImageIndex).src + '") 50% 50% / contain no-repeat rgba(255, 255, 255, 0)',
          'z-index': 100
        }).show();
  ...
  (function($){
    ...
    $.fn.fixed = function(ctx){
      try{
        var picPoint={leftOffset:0,topOffset:0,leftOffset2:0,topOffset2:0};
        var img = new Image();
        if(ctx.slider.image1.css('background-image') != 'none'){
          img.src=ctx.slider.image1.css('background-image').split('(')[1].split(')')[0];
          if((ctx.slider.width/img.naturalWidth)*img.naturalHeight > ctx.slider.height){
            picPoint.widthPic=(ctx.slider.height/img.naturalHeight)*img.naturalWidth;picPoint.heightPic=ctx.slider.height;
            picPoint.leftOffset=(ctx.slider.width-picPoint.widthPic)/2
          }else{
            picPoint.widthPic=ctx.slider.width;picPoint.heightPic=(ctx.slider.width/img.naturalWidth)*img.naturalHeight;
            picPoint.topOffset=(ctx.slider.height-picPoint.heightPic)/2
          }
        }
        if(ctx.slider.image2.css('background-image') != 'none'){
          img.src=ctx.slider.image2.css('background-image').split('(')[1].split(')')[0];
          if((ctx.slider.width/img.naturalWidth)*img.naturalHeight > ctx.slider.height){
            picPoint.widthPic2=(ctx.slider.height/img.naturalHeight)*img.naturalWidth;picPoint.heightPic2=ctx.slider.height;
            picPoint.leftOffset2=(ctx.slider.width-picPoint.widthPic2)/2
          }else{
            picPoint.widthPic2=ctx.slider.width;picPoint.heightPic2=(ctx.slider.width/img.naturalWidth)*img.naturalHeight;
            picPoint.topOffset2=(ctx.slider.height-picPoint.heightPic2)/2
          }
        }
        return picPoint;
      }catch(e){
  ...
  (function($) {
    flux.transitions.bars = function(fluxslider, opts) {
      return new flux.transition_grid(fluxslider, $.extend({
        ...
        renderTile: function(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
        try{
          var img = new Image(),widthPic=this.slider.width,heightPic=this.slider.height;
          leftOffset=parseInt("-"+leftOffset);
          if(this.slider.image1.css('background-image') != 'none'){
            img.src=this.slider.image1.css('background-image').split('(')[1].split(')')[0];
            if((this.slider.width/img.naturalWidth)*img.naturalHeight > this.slider.height){
              widthPic=(this.slider.height/img.naturalHeight)*img.naturalWidth;heightPic=this.slider.height;
              leftOffset=leftOffset+(this.slider.width-widthPic)/2
            }else{
              widthPic=this.slider.width;heightPic=(this.slider.width/img.naturalWidth)*img.naturalHeight;
            }
          }

          $(elem).css({
            'background': this.slider.image1.css('background-image')+' '+leftOffset+'px 50% no-repeat',
            'background-size':widthPic+'px '+heightPic+'px'
          }).css3({
    ...
    flux.transitions.bars3d = function(fluxslider, opts) {
      return new flux.transition_grid(fluxslider, $.extend({
        ...
        renderTile: function(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
          ...
          var img = new Image(),widthPic=this.slider.width,heightPic=this.slider.height,widthPic2=this.slider.width,heightPic2=this.slider.height;
           var leftPx=0;var topPx=0;var leftPx2=0;var topPx2=0;
         // leftOffset=topOffset;topOffset=topOffset;
          if(this.slider.image1.css('background-image') != 'none'){
            img.src=this.slider.image1.css('background-image').split('(')[1].split(')')[0];
            if((this.slider.width/img.naturalWidth)*img.naturalHeight > this.slider.height){
              widthPic=(this.slider.height/img.naturalHeight)*img.naturalWidth;heightPic=this.slider.height;
             leftPx=(this.slider.width-widthPic)/2
            }else{
              widthPic=this.slider.width;heightPic=(this.slider.width/img.naturalWidth)*img.naturalHeight;
              topPx=(this.slider.height-heightPic)/2
            }
          }
          if(this.slider.image2.css('background-image') != 'none'){
            img.src=this.slider.image2.css('background-image').split('(')[1].split(')')[0];
            if((this.slider.width/img.naturalWidth)*img.naturalHeight > this.slider.height){
              widthPic2=(this.slider.height/img.naturalHeight)*img.naturalWidth;heightPic2=this.slider.height;
              leftPx2=(this.slider.width-widthPic2)/2
            }else{
              widthPic2=this.slider.width;heightPic2=(this.slider.width/img.naturalWidth)*img.naturalHeight;
              topPx2=(this.slider.height-heightPic2)/2
            }
          }
          var bar = $('<div class="bar-'+colIndex+'"></div>').css({
              ...
              'background-position': parseInt('-'+leftOffset)+leftPx+'px '+topPx+'px',
              'background-size':widthPic+'px '+heightPic+'px',
              'background-repeat': 'no-repeat'
            }).css3({
            ...
            bar2 = $(bar.get(0).cloneNode(false)).css({
              ...
              'background-position': parseInt('-'+leftOffset)+leftPx2+'px '+topPx2+'px',
              'background-size':widthPic2+'px '+heightPic2+'px'
            }).css3({
    ...
    flux.transitions.blocks = function(fluxslider, opts) {
      return new flux.transition_grid(fluxslider, $.extend({
        ...
        renderTile: function(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
          ...
          var img = new Image(),widthPic=this.slider.width,heightPic=this.slider.height;
          leftOffset=parseInt("-"+leftOffset);topOffset=parseInt("-"+topOffset);
          if(this.slider.image1.css('background-image') != 'none'){
            img.src=this.slider.image1.css('background-image').split('(')[1].split(')')[0];
            if((this.slider.width/img.naturalWidth)*img.naturalHeight > this.slider.height){
              widthPic=(this.slider.height/img.naturalHeight)*img.naturalWidth;heightPic=this.slider.height;
              ...
              widthPic=this.slider.width;heightPic=(this.slider.width/img.naturalWidth)*img.naturalHeight;
              topOffset=topOffset+(this.slider.height-heightPic)/2
            }
          }
        ...
        setup: function() {
          ...
          for(var i=0; i<blockCount; i++)
          {
            ...
            var block = $('<div></div>').attr('class', 'block block-'+i).css({
              ...
              'background': this.slider.image1.css('background-image')+' 50% 50% / contain no-repeat',
    ...
    flux.transitions.cube = function(fluxslider, opts) {
      return new flux.transition(fluxslider, $.extend({
        ...
        setup: function() {
          ...
          var picPoint= $.fn.fixed(this);

          var currentFace = $('<div class="face current"></div>').css($.extend(css, {
            background: this.slider.image1.css('background-image'),
            'background-position': picPoint.leftOffset+'px '+picPoint.topOffset+'px',
            'background-size':picPoint.widthPic+'px '+picPoint.heightPic+'px',
            'background-repeat': 'no-repeat'
          })).css3({
            'backface-visibility': 'hidden'
          });
          ...
          var nextFace = $('<div class="face next"></div>').css($.extend(css, {
            ...
            'background-position': picPoint.leftOffset2+'px '+picPoint.topOffset2+'px',
            'background-size':picPoint.widthPic2+'px '+picPoint.heightPic2+'px',
            'background-repeat': 'no-repeat'
          })).css3({
    ...
    flux.transitions.tiles3d = function(fluxslider, opts) {
      return new flux.transition_grid(fluxslider, $.extend({
        ...
        renderTile: function(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
          ...
          var img = new Image(),widthPic=this.slider.width,heightPic=this.slider.height,widthPic2=this.slider.width,heightPic2=this.slider.height;
          var leftOffset2=parseInt("-"+leftOffset);var topOffset2=parseInt("-"+topOffset);
          leftOffset=parseInt("-"+leftOffset);topOffset=parseInt("-"+topOffset);
          if(this.slider.image1.css('background-image') != 'none'){
            img.src=this.slider.image1.css('background-image').split('(')[1].split(')')[0];
            if((this.slider.width/img.naturalWidth)*img.naturalHeight > this.slider.height){
          ...
          if(this.slider.image2.css('background-image') != 'none'){
            ...
            if((this.slider.width/img.naturalWidth)*img.naturalHeight > this.slider.height){
              widthPic2=(this.slider.height/img.naturalHeight)*img.naturalWidth;heightPic2=this.slider.height;
             leftOffset2=leftOffset2+(this.slider.width-widthPic2)/2
            }else{
              widthPic2=this.slider.width;heightPic2=(this.slider.width/img.naturalWidth)*img.naturalHeight;
             topOffset2=topOffset2+(this.slider.height-heightPic2)/2
            }
          }
          ...
          var tile2 = $(tile.get(0).cloneNode(false)).css({
            'background-image': this.slider.image2.css('background-image'),
            'background-position': leftOffset2+'px '+topOffset2+'px',
            'background-size':widthPic2+'px '+heightPic2+'px'
      ...
      return new flux.transition(fluxslider, $.extend({
        ...
        setup: function() {
          ...
          var tab = $('<div class="tab"></div>').css({
            ...
            front = $('<div class="front"></div>').appendTo(tab).css({
              'background-image': this.slider.image1.css('background-image'),
              'background-size':picPoint.widthPic+'px '+picPoint.heightPic+'px',
              'background-repeat': 'no-repeat',
              'background-position': parseInt(this.options.direction == 'left' ? '-'+(this.slider.image1.width()/2) : 0)+picPoint.leftOffset+'px '+picPoint.topOffset+'px',
              width: '100%',
              height: '100%',
              position: 'absolute',
              top: '0',
            ...
            back = $('<div class="back"></div>').appendTo(tab).css({
              'background-image': this.slider.image2.css('background-image'),
              'background-size':picPoint.widthPic2+'px '+picPoint.heightPic2+'px',
              'background-repeat': 'no-repeat',
              'background-position': parseInt(this.options.direction == 'left' ? 0 : '-'+(this.slider.image1.width()/2))+picPoint.leftOffset2+'px '+picPoint.topOffset2+'px',
              width: '100%',
              height: '100%',
              position: 'absolute',
              top: '0',
            ...
            current = $('<div class="current"></div>').css({
              ...
              'background-size':picPoint.widthPic+'px '+picPoint.heightPic+'px',
              'background-repeat': 'no-repeat',
              'background-position': parseInt(this.options.direction == 'left' ? 0 : '-'+(this.slider.image1.width()/2))+picPoint.leftOffset+'px '+picPoint.topOffset+'px',
              'z-index':100
            }),
        ...
        setup: function() {
          var width = this.slider.image1.width(),
            ...
            currentImage = $('<div class="current"></div>').css({
              ...
              top: '0px',
              left: '0px',
              background: this.slider[this.options.direction == 'left' ? 'image1' : 'image2'].css('background-image')+' 50% 50% / contain no-repeat'
            }).css3({
              'backface-visibility': 'hidden'
            }),
            ...
            nextImage = $('<div class="next"></div>').css({
              ...
              top: '0px',
              left: width+'px',
              background: this.slider[this.options.direction == 'left' ? 'image2' : 'image1'].css('background-image')+' 50% 50% / contain no-repeat'
            }).css3({
              'backface-visibility': 'hidden'
            });
